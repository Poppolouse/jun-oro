# 📊 İstatistikler ve Analiz

Jun-Oro platformunda oyun istatistiklerinizi detaylı bir şekilde inceleyebilir, oyun alışkanlıklarınızı analiz edebilir ve kişisel oyun verilerinizi görsel olarak takip edebilirsiniz. Bu rehberde istatistikler sayfasının tüm özelliklerini nasıl kullanabileceğinizi adım adım anlatıyorum.

## 🎯 İstatistikler Sayfasına Erişim

İstatistiklerinize ulaşmak için:

1. Ana menüden **"📊 İstatistikler"** seçeneğine tıklayın
2. Veya sol kenar çubuğundaki "İstatistikler" ikonuna tıklayın
3. İstatistikler sayfası size genel bir bakış sunacak şekilde açılacaktır

## 📈 Genel Bakış Paneli

### Ana İstatistikler
İstatistikler sayfasının üst kısmında genel oyun verilerinizi özetleyen kartlar bulunur:

- 🎮 **Toplam Oyun Sayısı**: Kütüphanenizdeki toplam oyun miktarı
- ✅ **Tamamlanan Oyunlar**: Bitirdiğiniz oyunların sayısı ve yüzdesi
- ⏱️ **Toplam Oynama Süresi**: Tüm oyunlarda harcadığınız toplam zaman
- 🎯 **Ortalama Tamamlama Süresi**: Bir oyunu bitirmek için ortalama harcadığınız zaman
- 📅 **Aktif Günler**: Oyun oynadığınız farklı gün sayısı

> 💡 **İpucu**: Bu kartlara tıklayarak ilgili detaylara hızlıca ulaşabilirsiniz!

### Zaman Dilimi Filtreleri
İstatistiklerinizi farklı zaman aralıklarında görüntüleyebilirsiniz:

- 📅 **Tüm Zamanlar**: Hesabınızı oluşturduğunuz günden bugüne tüm veriler
- 📅 **Bu Yıl**: Yılbaşından bugüne olan istatistikler
- 📅 **Bu Ay**: Mevcut aydaki oyun aktiviteleriniz
- 📅 **Son 30 Gün**: Son bir aydaki oyun alışkanlıklarınız
- 📅 **Özel Aralık**: Başlangıç ve bitiş tarihlerini kendiniz belirleyin

## 🎮 Oyun İstatistikleri

### Oyun Dağılımı
Oyunlarınızın farklı kategorilere göre dağılımını görsel olarak inceleyin:

#### Platform Dağılımı
- 🖥️ **PC**: Bilgisayarınızda oynadığınız oyunlar
- 🎮 **PlayStation**: PlayStation platformlarındaki oyunlar
- 🎯 **Xbox**: Xbox ekosistemindeki oyunlar
- 📱 **Mobil**: Mobil cihazlardaki oyunlar
- 🎲 **Diğer**: Diğer platformlardaki oyunlar

#### Tür Dağılımı
- 🎯 **Aksiyon**: Aksiyon oyunlarınızın sayısı ve yüzdesi
- 🎭 **RPG**: Rol yapma oyunlarınız
- 🧩 **Bulmaca**: Zeka ve bulmaca oyunlarınız
- 🏁 **Yarış**: Yarış oyunlarınız
- 🎪 **Strateji**: Strateji oyunlarınız
- 🎨 **Diğer**: Diğer türlerdeki oyunlarınız

> 📊 **Bilgi**: Pasta grafiklere tıklayarak dilimleri büyütebilir ve detayları görebilirsiniz.

### Oynama Süreleri
#### En Çok Oynanan Oyunlar
En çok zaman harcadığınız oyunların listesi:

1. **Oyun Adı** - Toplam süre - Son oynama tarihi
2. **Oyun Adı** - Toplam süre - Son oynama tarihi
3. **Oyun Adı** - Toplam süre - Son oynama tarihi

> 🕐 **İpucu**: Oyun adına tıklayarak oyun detay sayfasına ulaşabilirsiniz.

#### Oynama Grafiği
Zaman içindeki oynama alışkanlıklarınızı gösteren çizgi grafik:

- **X Ekseni**: Zaman (gün/hafta/ay)
- **Y Ekseni**: Oynama süresi (saat)
- **Renkli Çizgiler**: Farklı oyunların oynama süreleri

Grafiği etkileşimli olarak kullanabilir:
- 🖱️ **Imleç üzerine getirme**: O anki detayları gösterir
- 🔍 **Yakınlaştırma**: Belirli bir zaman dilimine odaklanma
- 📷 **Ekran görüntüsü**: Grafiği kaydetme

## 🏆 Başarı ve Tamamlama İstatistikleri

### Tamamlama Oranları
Oyunlarınızın tamamlama durumlarını farklı açılardan inceleyin:

#### Genel Tamamlama Grafiği
- ✅ **Tamamlandı**: Bitirdiğiniz oyunlar
- 🎮 **Devam Ediyor**: Şu anda oynadığınız oyunlar
- ⏸️ **Bırakıldı**: Yarıda bıraktığınız oyunlar
- 📋 **Planlandı**: Oynamayı planladığınız oyunlar

#### Platform Bazında Tamamlama
Her platformdaki tamamlama oranlarınız:
- PC: %75 tamamlama oranı
- PlayStation: %60 tamamlama oranı
- Xbox: %85 tamamlama oranı

### Başarı Rozetleri
Kazandığınız başarıları kategorilere göre görüntüleyin:

- 🏅 **Bronz Başarılar**: Temel başarılar
- 🥈 **Gümüş Başarılar**: Orta zorlukta başarılar
- 🥇 **Altın Başarılar**: Zor başarılar
- 💎 **Elmas Başarılar**: Nadir başarılar
- 👑 **Platin Başarılar**: Tüm başarıları tamamlama

> 🎯 **Hedef**: Platform bazında tamamlama oranlarınızı artırmak için yeni hedefler belirleyin!

## 📅 Zaman Analizi

### Oynama Alışkanlıkları
#### Haftalık Analiz
Haftanın günlerine göre oynama dağılımınız:

- **Pazartesi**: 2.5 saat
- **Salı**: 1.8 saat
- **Çarşamba**: 3.2 saat
- **Perşembe**: 2.1 saat
- **Cuma**: 4.5 saat
- **Cumartesi**: 6.8 saat
- **Pazar**: 7.2 saat

> 📊 **Bilgi**: En çok hafta sonu oynadığınızı görüyoruz. Bu bilgiyi oyun planlamanızda kullanabilirsiniz!

#### Aylık Analiz
Son 12 aydaki oynama trendleriniz:
- 📈 **Artış Trendi**: Oynama sürenizin arttığı aylar
- 📉 **Düşüş Trendi**: Daha az oyun oynadığınız aylar
- 📊 **Kararlı**: Benzer seviyede kaldığınız aylar

### Gün İçi Analiz
Günün saatlerine göre oynama dağılımınız:
- 🌅 **Sabah (06:00-12:00)**: Genellikle az oynama
- 🌞 **Öğleden Sonra (12:00-18:00)**: Orta seviye oynama
- 🌆 **Akşam (18:00-24:00)**: En çok oynanan zaman dilimi
- 🌙 **Gece (00:00-06:00)**: Nadir oynama

## 🎯 Kişisel Hedefler

### Hedef Oluşturma
Kendinize oyun hedefleri belirleyin ve takibini yapın:

#### Aylık Hedefler
- 📚 **Oyun Sayısı**: Bu ay 5 yeni oyun bitirme
- ⏱️ **Oynama Süresi**: Ayda 40 saat oyun oynama
- 🏆 **Başarı**: 50 yeni başarı kazanma
- 🎮 **Tür Çeşitliliği**: 3 farklı türde oyun oynama

#### Yıllık Hedefler
- 📊 **Backlog Temizleme**: Kütüphanedeki %20 oyunu bitirme
- 🌟 **Yeni Deneyimler**: 10 yeni türde oyun deneme
- 💰 **Bütçe Kontrolü**: Oyun harcamalarını limit altında tutma

### Hedef Takibi
Belirlediğiniz hedeflerin ilerlemesini gerçek zamanlı olarak takip edin:

- ✅ **Tamamlandı**: Hedefe ulaştınız
- 🔄 **Devam Ediyor**: Hedefe ulaşmak için çalışıyorsunuz
- ⚠️ **Risk Altında**: Hedefe ulaşma zorluğu yaşıyorsunuz
- ❌ **Başarısız**: Hedefe ulaşamadınız

> 💡 **İpucu**: Hedeflerinize ulaşamadığınızda sistem otomatik olarak yeni stratejiler önerebilir!

## 📊 Veri Dışa Aktarma

### Rapor Oluşturma
İstatistik verilerinizi farklı formatlarda dışa aktarabilirsiniz:

#### PDF Raporu
- 📄 **Özet Rapor**: Temel istatistiklerinizi içeren 1 sayfalık rapor
- 📊 **Detaylı Rapor**: Tüm analizleri içeren kapsamlı rapor
- 📈 **Görsel Rapor**: Grafikler ve görsellerle zenginleştirilmiş rapor

#### Veri Dosyaları
- 📊 **Excel (.xlsx)**: Tablo formatında detaylı veriler
- 📈 **CSV**: Analiz için ham veriler
- 📋 **JSON**: Programatik kullanım için yapılandırılmış veriler

### Otomatik Raporlar
Periyodik olarak otomatik raporlar alabilirsiniz:

- 📧 **E-posta ile**: Haftalık/aylık özet raporları
- 📱 **Bildirim ile**: Önemli kilometre taşlarında anlık bildirimler
- 📅 **Takvim ile**: Google Calendar'a rapor ekleme

## 🔍 Detaylı Analiz Araçları

### Karşılaştırma Modülü
Farklı zaman dilimlerini veya oyunları karşılaştırın:

#### Zaman Karşılaştırması
- Bu ay vs geçen ay oynama süreleri
- Bu yıl vs geçen yıl tamamlama oranları
- Son 3 ay vs önceki 3 ay trend analizi

#### Oyun Karşılaştırması
- İki oyunun oynama sürelerini karşılaştırma
- Aynı türdeki oyunların tamamlama süreleri
- Farklı platformlardaki performans analizi

### Tahminleme Sistemi
Mevcut verilerinize dayanarak gelecek tahminleri:

- 📈 **Tamamlama Tahmini**: Mevcut oyunlarınızı ne zaman bitireceğiniz
- 🎯 **Hedef Tahmini**: Belirlediğiniz hedeflere ulaşma zamanı
- 💰 **Bütçe Tahmini**: Gelecek oyun harcamalarınızın tahmini

## 🎨 Kişiselleştirme

### Görünüm Ayarları
İstatistikler sayfasını kendi zevkinize göre özelleştirin:

#### Tema Seçenekleri
- 🌞 **Açık Tema**: Parlak ve ferah görünüm
- 🌙 **Karanlık Tema**: Göz yormayan karanlık mod
- 🎨 **Özel Tema**: Kendi renk paletinizi oluşturun

#### Grafik Stilleri
- 📊 **Modern**: Şık ve minimalist grafikler
- 🎮 **Oyun Temalı**: Oyun estetiğine uygun grafikler
- 📈 **Profesyonel**: İş odaklı analitik grafikler

### Widget Yönetimi
İstediğiniz istatistik widget'larını sayfada düzenleyin:

- ➕ **Widget Ekle**: Yeni istatistik bileşeni ekleme
- 🔄 **Sırala**: Widget'ları yeniden düzenleme
- ❌ **Kaldır**: İhtiyacınız olmayan widget'ları gizleme
- 💾 **Kaydet**: Düzeninizi kaydetme

## 📱 Mobil Uygulama İstatistikleri

Mobil uygulama üzerinden de istatistiklerinize erişebilirsiniz:

- 📊 **Özet Widget'ları**: Ana ekranda hızlı istatistikler
- 📈 **Bildirimler**: İstatistiksel kilometre taşları için bildirimler
- 🔄 **Senkronizasyon**: Tüm cihazlarınızda veri tutarlılığı

## 🔗 Sosyal Paylaşım

İstatistiklerinizi arkadaşlarınızla paylaşabilirsiniz:

- 📸 **Grafik Paylaşımı**: İlgilendiğiniz grafikleri sosyal medyada paylaşma
- 🏆 **Başarı Paylaşımı**: Önemli başarıları duyurma
- 📊 **Yıllık Özet**: "Oyun Yılım" gibi özel raporları paylaşma

> 🎯 **Gizlilik**: Paylaşım ayarlarınızı istediğiniz zaman değiştirebilirsiniz.

## ❓ Sıkça Sorulan Sorular

**S: İstatistiklerim ne kadar sıklıkla güncellenir?**
C: Oyun oynama verileriniz gerçek zamanında, diğer istatistikler ise saatlik olarak güncellenir.

**S: Geçmiş verilerimi düzenleyebilir miyim?**
C: Oynama süreleri otomatik olarak kaydedilir, ancak oyun bilgilerini manuel olarak düzenleyebilirsiniz.

**S: İstatistiklerimi dışa aktarmanın ücretli midir?**
C: Temel dışa aktarma özellikleri ücretsizdir. Detaylı raporlar için premium üyelik gerekebilir.

**S: Birden fazla platformdaki verilerimi birleştirebilir miyim?**
C: Evet, tüm platform hesaplarınızı bağlayarak verilerinizi tek bir yerde birleştirebilirsiniz.

---

Bu rehberde Jun-Oro istatistikler özelliklerinin tamamını öğrendiniz. İstatistiklerinizi düzenli olarak takip ederek oyun alışkanlıklarınız hakkında değerli içgörüler elde edebilir ve gelecek hedeflerinizi daha etkili bir şekilde belirleyebilirsiniz.

Oyun istatistiklerinizle ilgili daha fazla yardıma ihtiyacınız olursa [Sorun Giderme](../troubleshooting.md) rehberimizi inceleyebilir veya destek ekibimizle iletişime geçebilirsiniz.