...
import { uploadAvatar, uploadPlatformLogo, uploadGameCover, handleUploadError } from '../middleware/upload.js';
  ...
  try {
    ...
    if (uploadService.testConnection) {
      ...
      res.json({
        ...
        message: isConnected ? 'R2 bağlantısı başarılı' : 'R2 bağlantısı başarısız'
      });
    } else {
      res.json({
        success: false,
...
router.post('/cors', async (req, res) => {
  try {
    ...
    if (uploadService.setBucketCors) {
      ...
      res.json({
        success: success,
        message: success ? 'CORS ayarları güncellendi' : 'CORS ayarları güncellenemedi'
      });
    } else {
      res.json({
        success: false,
...
const getUploadService = () => {
  if (!cloudflareR2.isConfigured()) {
    throw new Error('Cloudflare R2 is not configured. Please set up R2 environment variables.');
  }
  ...
  try {
    ...
    if (!cloudflareR2.isConfigured()) {
      return res.status(503).json({
        success: false,
        message: 'Presigned URL özelliği sadece Cloudflare R2 ile kullanılabilir'
      });
    }
    ...
    const presignedUrl = await cloudflareR2.generatePresignedUploadUrl(key, contentType);
