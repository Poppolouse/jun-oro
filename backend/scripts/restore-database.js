...
async function restoreDatabase(backupFilePath) {
  try {
    ...
    console.log(
      `   - Oyun Oturumları: ${backupData.data.gameSessions?.length || 0}`,
    );
    ...
    if (process.env.FORCE_RESTORE !== "true") {
      ...
      if (process.stdin.isTTY) {
        ...
        console.log(
          "❌ İnteraktif olmayan ortamda onay alınamadı. FORCE_RESTORE=true ortam değişkenini kullanın.",
        );
    ...
    await prisma.$transaction(async (tx) => {
      ...
      if (backupData.data.users?.length > 0) {
        ...
        console.log(
          `✅ ${backupData.data.users.length} kullanıcı geri yüklendi`,
        );
      }
      ...
      if (backupData.data.gameSessions?.length > 0) {
        ...
        console.log(
          `✅ ${backupData.data.gameSessions.length} oyun oturumu geri yüklendi`,
        );
      }

      if (backupData.data.userLibraries?.length > 0) {
        await tx.user_libraries.createMany({
          data: backupData.data.userLibraries,
        });
        console.log(
          `✅ ${backupData.data.userLibraries.length} kütüphane kaydı geri yüklendi`,
        );
      }
      ...
      if (backupData.data.notifications?.length > 0) {
        ...
        console.log(
          `✅ ${backupData.data.notifications.length} bildirim geri yüklendi`,
        );
      }
      ...
      if (backupData.data.changelog?.length > 0) {
        ...
        console.log(
          `✅ ${backupData.data.changelog.length} changelog geri yüklendi`,
        );
      }
    });
