...
function Header() {
  ...
  const [tutorialDropdownPosition, setTutorialDropdownPosition] = useState({ top: 0, right: 0 })
  ...
  useEffect(() => {
    function handleClickOutside(event) {
      ...
      if (tutorialDropdownRef.current && !tutorialDropdownRef.current.contains(event.target) && 
          tutorialButtonRef.current && !tutorialButtonRef.current.contains(event.target)) {
  ...
  return (
    <header className="h-20 bg-gradient-to-r from-[#1a1a2e]/90 via-[#2d1b69]/90 to-[#1a1a2e]/90 backdrop-blur-xl border-b border-purple-500/20 flex items-center justify-between px-8">
      ...
      <div className="flex items-center gap-4 cursor-pointer hover:opacity-80 transition-opacity" onClick={() => navigate('/')}>
        ...
        <div>
          <h1 className="text-2xl font-bold bg-gradient-to-r from-white to-gray-300 bg-clip-text text-transparent">Jun-Oro</h1>
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="relative z-[10000]">
          ...
          >
            ...
            <span className={`text-gray-400 text-sm transition-transform ${isProfileDropdownOpen ? 'rotate-180' : ''}`}>▼</span>
          </button>
