      ...
      <div className="text-center" id="updates-card" data-registry="1.0.B5">
        <div className="bg-slate-800/30 border border-slate-700/30 rounded-2xl p-8 md:p-12">
          <div
            ...
            data-registry="1.0.B5.1"
          >
            🛠️
          </div>
          <h3
            className="text-2xl font-bold text-white mb-4"
            data-registry="1.0.B5.2"
          >
    ...
    <div className="text-center" id="updates-card" data-registry="1.0.B5">
      <div className="bg-slate-800/30 border border-slate-700/30 rounded-2xl p-8 md:p-12">
        ...
        <p
          ...
          data-registry="1.0.B5.3"
        >
          Burada şu an üzerinde çalıştığımız özelliklerin ilerlemesini ve son
          tamamlanan güncellemeleri görebilirsiniz.
        </p>
        ...
        >
          ...
          {current.length === 0 ? (
            ...
            current.map((item) => (
              <div
                key={item.id}
                className="mb-6 p-4 rounded-xl bg-slate-800/40 border border-slate-700/40"
              >
                <div className="flex items-center justify-between gap-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <h4
                        ...
                        data-registry={`1.0.B5.4.${item.id}.title`}
                      >
                        {item.title}
                      </h4>
                      <span
                        className={`text-xs px-2 py-1 rounded-full text-black ${statusColors[item.status] || "bg-gray-500"}`}
                        id={`update-status-${item.id}`}
                        data-registry={`1.0.B5.4.${item.id}.status`}
                      >
                        {statusLabels[item.status] || item.status}
                      </span>
                      {item.type && (
                        <span
                          className={`text-xs px-2 py-1 rounded-full text-white ${typeColors[item.type] || "bg-gray-600"}`}
                          id={`update-type-${item.id}`}
                          data-registry={`1.0.B5.4.${item.id}.type`}
                        >
                      ...
                      {item.priority && (
                        <span
                          className={`text-xs px-2 py-1 rounded-full text-white ${priorityColors[item.priority] || "bg-gray-600"}`}
                          id={`update-priority-${item.id}`}
                          data-registry={`1.0.B5.4.${item.id}.priority`}
                        >
                    ...
                    {item.targetDate && (
                      <p className="text-gray-500 text-xs mt-1">
                        Hedef:{" "}
                        {new Date(item.targetDate).toLocaleDateString("tr-TR")}
                      </p>
                    )}
                  </div>
                  ...
                  {item.substeps && item.substeps.length > 0 && (
                    ...
                    >
                      ...
                      <span
                        className={`transform transition-transform ${expandedId === item.id ? "rotate-180" : ""}`}
                      >
                        ▼
                      </span>
                    </button>
                ...
                {expandedId === item.id &&
                  ...
                  item.substeps.length > 0 && (
                    <div
                      ...
                      id={`substeps-list-${item.id}`}
                      data-registry={`1.0.B5.4.${item.id}.substeps-list`}
                    >
                        ...
                        >
                          <div className="flex-1">
                            <div className="flex items-center gap-2">
                              ...
                              <span
                                className={`text-xs px-2 py-1 rounded-full text-black ${statusColors[substep.status] || "bg-gray-500"}`}
                              >
                          ...
                          <div className="ml-4 min-w-[80px]">
                            <div className="w-full h-2 bg-slate-700 rounded-full overflow-hidden">
                              <div
                                ...
                                style={{
                                  width: `${Math.min(100, Math.max(0, substep.progress || 0))}%`,
                                }}
                              />
                            </div>
                            <div className="text-right text-xs text-gray-400 mt-1">
                              %{Math.round(substep.progress || 0)}
                            </div>
                          </div>
                        </div>
                      ))}
        ...
        >
          ...
          {plannedUpdates.length === 0 ? (
            <div className="text-gray-400 text-sm">
              Henüz planlanan güncelleme bulunmuyor.
            </div>
          ) : (
            plannedUpdates.map((update) => (
              <div
                key={update.id}
                className="mb-4 p-4 rounded-lg bg-slate-800/40 border border-slate-700/40"
              >
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <div
                      className="text-white font-medium"
                      id={`planned-title-${update.id}`}
                      data-registry={`1.0.B5.3.${update.id}.title`}
                    >
                      {update.title}
                    </div>
                    {update.type && (
                      <span
                        className={`text-xs px-2 py-1 rounded-full text-white ${typeColors[update.type] || "bg-gray-600"}`}
                      >
                ...
                {update.description && (
                  <div
                    ...
                    id={`planned-desc-${update.id}`}
                    data-registry={`1.0.B5.3.${update.id}.desc`}
                  >
                    {update.description}
                  </div>
                )}
                {update.targetDate && (
                  <div className="text-xs text-blue-400">
                    Hedef Tarih:{" "}
                    {new Date(update.targetDate).toLocaleDateString("tr-TR")}
                  </div>
                )}
              </div>
            ))
        ...
        <div
          ...
          id="updates-recent"
          data-registry="1.0.B5.5"
        >
          <h4 className="text-lg font-semibold text-white mb-3">
            Son Tamamlanan Güncellemeler
          </h4>
          {upcoming.length === 0 ? (
            <div className="text-gray-400 text-sm">
              Henüz tamamlanan güncelleme bulunmuyor.
            </div>
          ) : (
            <ul className="space-y-3">
              {upcoming.map((it, idx) => (
                <li
                  key={it.id}
                  className="p-3 rounded-lg bg-slate-800/40 border border-slate-700/40 flex items-start gap-3"
                >
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <div
                        className="text-white font-medium"
                        id={`recent-title-${it.id}`}
                        data-registry={`1.0.B5.5.${it.id}.title`}
                      >
                        {it.title}
                      </div>
                      {it.status && (
                        <span
                          className={`text-xs px-2 py-1 rounded-full text-black ${statusColors[it.status] || "bg-gray-500"}`}
                        >
                      ...
                      {it.type && (
                        <span
                          className={`text-xs px-2 py-1 rounded-full text-white ${typeColors[it.type] || "bg-gray-600"}`}
                        >
                    ...
                    {it.description && (
                      <div
                        ...
                        id={`recent-desc-${it.id}`}
                        data-registry={`1.0.B5.5.${it.id}.desc`}
                      >
                        {it.description}
                      </div>
                    )}
                    {it.completedAt && (
                      <div className="text-xs text-gray-500 mt-1">
                        Tamamlandı:{" "}
                        {new Date(it.completedAt).toLocaleDateString("tr-TR")}
                      </div>
                    )}
                  </div>
                </li>
