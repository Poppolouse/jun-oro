...
export default function UpdatesAdmin() {
  ...
  const handleUpdateUpdate = async () => {
    try {
      setSaving(true)
      const response = await updatesApi.updateUpdate(editingUpdate.id, formData)
      if (response.success) {
        await fetchUpdates()
  ...
  const handleCreateSubstep = async () => {
    ...
    setSaving(true)
    try {
      const response = await updatesApi.createSubstep(selectedUpdateId, substepFormData)
      if (response.success) {
        await fetchUpdates()
        resetSubstepForm()
      } else {
  ...
  const handleUpdateSubstep = async () => {
    if (!substepFormData.title.trim() || !editingSubstep || !selectedUpdateId) return
    
    setSaving(true)
    try {
      const response = await updatesApi.updateSubstep(selectedUpdateId, editingSubstep.id, substepFormData)
      if (response.success) {
        await fetchUpdates()
        resetSubstepForm()
      } else {
  ...
  const handleDeleteSubstep = async (substepId) => {
    ...
    const parentUpdate = updates.find(update => 
      update.substeps && update.substeps.some(substep => substep.id === substepId)
    )
    ...
    setSaving(true)
    try {
      const response = await updatesApi.deleteSubstep(parentUpdate.id, substepId)
      if (response.success) {
        await fetchUpdates()
      } else {
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-2xl font-bold text-white">Güncel Geliştirmeler Yönetimi</h2>
          <p className="text-gray-400">Ana sayfada görünen güncellemeleri buradan yönetebilirsiniz.</p>
        </div>
        <button
      ...
      {(showCreateForm || editingUpdate) && (
        <div className="mb-6 p-6 bg-slate-800/50 border border-slate-700/50 rounded-xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm text-gray-300 mb-1">Versiyon</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-sm text-gray-300 mb-1">Öncelik</label>
              <select
            ...
            <div>
              <label className="block text-sm text-gray-300 mb-1">İlerleme (%)</label>
              <input
                type="number"
                min="0"
                max="100"
          ...
          <div className="flex items-center gap-3 mt-6">
            ...
            >
              {saving ? 'Kaydediliyor...' : (editingUpdate ? 'Güncelle' : 'Oluştur')}
            </button>
            <button
              onClick={cancelEdit}
      ...
      {showSubstepForm && (
        <div className="mb-6 p-6 bg-slate-800/50 border border-slate-700/50 rounded-xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setSubstepFormData(prev => ({ ...prev, status: e.target.value }))}
                className="w-full px-3 py-2 bg-slate-700/50 border border-slate-600/50 rounded-lg text-white"
              >
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setSubstepFormData(prev => ({ ...prev, progress: parseInt(e.target.value) || 0 }))}
                className="w-full px-3 py-2 bg-slate-700/50 border border-slate-600/50 rounded-lg text-white"
              />
            </div>
          ...
          <div className="flex items-center gap-3 mt-6">
            <button
              onClick={editingSubstep ? handleUpdateSubstep : handleCreateSubstep}
              disabled={saving || !substepFormData.title.trim()}
              className="px-4 py-2 bg-green-600 hover:bg-green-700 disabled:bg-gray-600 text-white rounded-lg transition-colors"
            >
              {saving ? 'Kaydediliyor...' : (editingSubstep ? 'Güncelle' : 'Oluştur')}
            </button>
            <button
      ...
      <div className="space-y-4">
        {updates.length === 0 ? (
          ...
          updates.map(update => (
            <div key={update.id} className="p-4 bg-slate-800/30 border border-slate-700/30 rounded-xl">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className={`px-2 py-1 text-xs rounded-full ${
                      update.status === 'completed' ? 'bg-green-600 text-white' :
                      update.status === 'in_progress' ? 'bg-blue-600 text-white' :
                      update.status === 'planned' ? 'bg-yellow-600 text-black' :
                      'bg-red-600 text-white'
                    }`}>
                      {statuses.find(s => s.value === update.status)?.label || update.status}
                    </span>
                    <span className={`px-2 py-1 text-xs rounded-full ${
                      update.priority === 'critical' ? 'bg-red-500 text-white' :
                      update.priority === 'high' ? 'bg-orange-500 text-white' :
                      update.priority === 'medium' ? 'bg-yellow-500 text-black' :
                      'bg-gray-500 text-white'
                    }`}>
                      {priorities.find(p => p.value === update.priority)?.label || update.priority}
                    </span>
                  ...
                  <div className="flex items-center gap-4 text-xs text-gray-400">
                    <span>Tür: {types.find(t => t.value === update.type)?.label || update.type}</span>
                    ...
                    <span>Oluşturuldu: {new Date(update.createdAt).toLocaleDateString('tr-TR')}</span>
                  </div>
                    ...
                    <div className="mt-3">
                      <div className="w-full h-2 bg-slate-700 rounded-full overflow-hidden">
                        <div
                          ...
                          style={{ width: `${Math.min(100, Math.max(0, update.progress))}%` }}
                        />
                      </div>
                    </div>
                  )}
                  ...
                  {expandedUpdate === update.id && (
                    <div className="mt-4 p-4 bg-slate-900/50 rounded-lg border border-slate-600/30">
                      <div className="flex items-center justify-between mb-3">
                        <h4 className="text-sm font-semibold text-white">Alt Adımlar</h4>
                        <button
                      ...
                      {update.substeps && update.substeps.length > 0 ? (
                        <div className="space-y-2">
                          {update.substeps.map(substep => (
                            <div key={substep.id} className="flex items-center justify-between p-3 bg-slate-800/50 rounded border border-slate-600/20">
                              <div className="flex-1">
                                <div className="flex items-center gap-2 mb-1">
                                  <span className="text-sm font-medium text-white">{substep.title}</span>
                                  <span className={`px-2 py-0.5 text-xs rounded-full ${
                                    substep.status === 'completed' ? 'bg-green-600 text-white' :
                                    substep.status === 'in_progress' ? 'bg-blue-600 text-white' :
                                    'bg-gray-600 text-white'
                                  }`}>
                                    {substep.status === 'completed' ? 'Tamamlandı' :
                                     substep.status === 'in_progress' ? 'Devam Ediyor' : 'Bekliyor'}
                                  </span>
                                </div>
                                {substep.description && (
                                  ...
                                  <div className="mt-2">
                                    <div className="w-full h-1 bg-slate-700 rounded-full overflow-hidden">
                                      <div
                                        ...
                                        style={{ width: `${Math.min(100, Math.max(0, substep.progress))}%` }}
                                      />
                                    </div>
                                    <span className="text-xs text-gray-400 mt-1">%{substep.progress}</span>
                                  </div>
                                )}
                              </div>
                              <div className="flex items-center gap-1 ml-3">
                                <button
                                  onClick={() => startSubstepEdit(substep, update.id)}
