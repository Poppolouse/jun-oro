...
function ArkadeSidebar() {
  ...
  const { activeSession, sessionTimer, isRunning, formatTime, stopSession, toggleSession, getUserStats } = useActiveSession()
  ...
  useEffect(() => {
    const loadGames = async () => {
      ...
      const newGamesThisWeek = libraryGames.filter(g => 
        g.libraryInfo?.addedAt && g.libraryInfo.addedAt > oneWeekAgo
      ).length
  ...
  const getGameCategory = (game) => game.libraryInfo?.category || game.category || 'wishlist'
  ...
  const navigationItems = [
    { name: 'Dashboard', icon: '🏠', active: activeTab === 'dashboard', count: null, route: '/arkade' },
    { name: 'Kütüphane', icon: '📚', active: activeTab === 'library', count: games.length, route: '/arkade/library' },
    { name: 'Backlog', icon: '📋', active: activeTab === 'backlog', count: games.filter(g => getGameCategory(g) === 'wishlist' && getGameStatus(g) === 'backlog').length, route: '/backlog' },
    { name: 'İstatistikler', icon: '📊', active: activeTab === 'stats', count: null, route: '/stats' },
    { name: 'İstek listesi', icon: '💝', active: activeTab === 'wishlist', count: games.filter(g => getGameCategory(g) === 'wishlist' && getGameStatus(g) !== 'backlog').length, route: '/wishlist' },
    { name: 'Galeri', icon: '📷', active: activeTab === 'gallery', count: null, route: '/gallery' }
  ]
              ...
              <div className="relative z-10">
                ...
                >
                  ...
                  >
                    ...
                    >
                      {activeSession.name || activeSession.title || 'Bilinmeyen Oyun'}
                    </h4>
                    <p 
                    ...
                    >
                      {(activeSession.genre || activeSession.genres?.[0] || 'Oyun')}
                    </p>
                    <div 
                ...
                >
                  ...
                  >
                    ...
                    <span className="text-white font-medium">{formatTime(sessionTimer)}</span>
                  </div>
                  <div 
                  ...
                  {activeSession.progress && (
                    <>
                      ...
                      >
                        ...
                        <span className="text-purple-400 font-medium">{activeSession.progress}%</span>
                      </div>
                    </>
                  )}
                </div>
              ...
              {item.count !== null && (
                <span 
                  ...
                  className={`px-2 py-1 rounded-full text-xs font-bold ${
                    item.active ? 'bg-[#00ff88]/20 text-[#00ff88]' : 'bg-white/10 text-gray-400'
                  }`}
                >
                  {item.count}
                </span>
          ...
          >
            ...
            >
              ...
              >
                ...
                <span className="text-white font-semibold text-xs">{games.length}</span>
              </div>
              <div 
              ...
              >
                ...
                <span className="text-[#00ff88] font-semibold text-xs">{games.filter(g => getGameCategory(g) === 'playing').length}</span>
              </div>
              <div 
              ...
              >
                ...
                <span className="text-green-400 font-semibold text-xs">{games.filter(g => getGameCategory(g) === 'completed').length}</span>
              </div>
              <div 
              ...
              >
                ...
                <span className="text-orange-400 font-semibold text-xs">{games.filter(g => getGameCategory(g) === 'wishlist' && getGameStatus(g) === 'backlog').length}</span>
              </div>
              <div 
              ...
              >
                ...
                <span className="text-red-400 font-semibold text-xs">{games.filter(g => getGameCategory(g) === 'dropped').length}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        >
          ...
          >
            ...
            >
              ...
              >
                {weeklyStats.playTime > 0 ? `${Math.round(weeklyStats.playTime / 60)}h` : '0h'}
              </div>
              <div 
