...
function ArkadeHeader() {
  ...
  useEffect(() => {
    const loadNotifications = () => {
      try {
        ...
        const notificationsWithReadStatus = storedNotifications.map(notification => ({
          ...notification,
  ...
  const markNotificationAsRead = (notificationId) => {
    ...
    try {
      ...
      localStorage.setItem(`notificationReadStatus_${user.id}`, JSON.stringify(userReadStatus))
    ...
    >
      ...
      >
        ...
        >
          ...
          <div data-registry="H1.1.2" id="logo-text">
            <h1 className="text-2xl font-bold bg-gradient-to-r from-green-400 to-blue-400 bg-clip-text text-transparent">Arkade</h1>
          ...
          >
            <div className="bg-gray-900/95 backdrop-blur-xl border border-gray-700/50 rounded-lg p-3 shadow-xl min-w-[280px]">
              <div className="text-center">
                ...
                <h4 className="text-white font-semibold text-sm mb-2">Akıllı Arama Sistemi</h4>
                <p className="text-gray-400 text-xs leading-relaxed">
                  AI destekli oyun arama, otomatik tamamlama ve akıllı filtreler yakında geliyor!
                </p>
                <div className="mt-2 flex items-center justify-center gap-1 text-green-400">
                  <div className="w-1.5 h-1.5 bg-green-400 rounded-full animate-pulse"></div>
                  <span className="text-xs font-medium">Geliştirme Aşamasında</span>
                </div>
              </div>
            </div>
          </div>
      ...
      >
        ...
        >
          ...
          {isNotificationDropdownOpen && createPortal(
            <div 
                    ...
                    >
                      <div className="flex items-start gap-3">
                        <div className={`w-2 h-2 rounded-full mt-2 flex-shrink-0 ${
                          !notification.read ? 'bg-green-500' : 'bg-gray-600'
                        }`}></div>
          ...
          >
            ...
            >
              {user?.profileImage ? (
                ...
                <span className="text-white font-bold text-sm">{user?.avatar || user?.username?.[0]?.toUpperCase() || 'U'}</span>
              )}
            </div>
            <div 
                 ...
                 <div className="flex items-center gap-3">
                   <div className="w-12 h-12 rounded-full overflow-hidden bg-gradient-to-br from-green-500 to-blue-500 flex items-center justify-center shadow-lg">
                     {user?.profileImage ? (
                       ...
                       <span className="text-white font-bold">{user?.avatar || user?.username?.[0]?.toUpperCase() || 'U'}</span>
                     )}
                   </div>
                   <div>
                     <div className="text-white font-medium">{user?.username}</div>
                     <div className="text-gray-400 text-sm">{user?.role === 'admin' ? 'Admin Kullanıcı' : 'Standart Kullanıcı'}</div>
      ...
      {isNotificationDetailOpen && selectedNotification && createPortal(
        <div 
          data-registry="H4" 
          ...
          >
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-white font-bold text-lg">Bildirim Detayı</h3>
              </div>
              <button
            ...
            >
              <div>
                <h4 className="text-white font-semibold text-lg mb-2">{selectedNotification.title}</h4>
              ...
              <div className="flex items-center gap-4 text-sm text-gray-400 pt-4 border-t border-gray-700">
                ...
                <span className={`px-2 py-1 rounded-full text-xs ${
                  selectedNotification.read ? 'bg-gray-600 text-gray-300' : 'bg-green-500/20 text-green-400'
                }`}>
              ...
              {selectedNotification.details && (
                <div className="bg-gray-800/50 rounded-lg p-4">
                  ...
                  <p className="text-gray-300 text-sm">{selectedNotification.details}</p>
                </div>
              )}
            </div>
