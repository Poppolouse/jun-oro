...
class UserLibraryService {
  ...
  _parsePlaytime(playtimeStr) {
    ...
    const patterns = [
      ...
      { regex: /(\d+(?:[.,]\d+)?)\s*(?:dakika|minute|min|dk|m)/, multiplier: 1 },
  ...
  _buildMetadataTags(gameData) {
    try {
      ...
      const campaigns = Array.isArray(gameData.campaigns) ? gameData.campaigns : undefined
      const selectedDlcs = Array.isArray(gameData.selectedDlcs) ? gameData.selectedDlcs : undefined
      const steamDlcs = Array.isArray(gameData.steamDlcs) ? gameData.steamDlcs : undefined
      const gameVariants = gameData.gameVariants && typeof gameData.gameVariants === 'object' ? gameData.gameVariants : undefined
      const dlcSource = (steamDlcs && steamDlcs.length > 0) ? 'steam' : (selectedDlcs && selectedDlcs.length > 0) ? 'igdb' : undefined
        ...
        try {
          ...
          if (id) {
            ...
            try {
              ...
              console.warn('Backend user not found or unreachable; using local user', e)
  ...
  async getUserLibrary(filters = {}) {
    try {
      ...
      const response = await libraryApi.getLibrary(this.currentUser.id, filters)
  ...
  async addGameToLibrary(gameData, category = 'playing') {
    try {
      ...
      const gameDataForApi = {
        ...
        genres: gameData.genres ? gameData.genres.map(genre => 
          typeof genre === 'object' ? genre.name || genre.slug || String(genre) : String(genre)
        ) : undefined,
        // Platforms alanını string array'e çevir
        platforms: gameData.platforms ? gameData.platforms.map(platform => 
          typeof platform === 'object' ? platform.name || platform.slug || String(platform) : String(platform)
        ) : undefined,
      ...
      console.log(`🎮 Oyun kütüphaneye eklendi: ${gameData.name} (${normalizedCategory})`)
      return response.data
    } catch (error) {
  ...
  async removeGameFromLibrary(gameId) {
    try {
      ...
      const response = await libraryApi.removeFromLibrary(this.currentUser.id, gameId)
  ...
  async changeGameCategory(gameId, newCategory) {
    try {
      ...
      const response = await libraryApi.updateEntry(this.currentUser.id, gameId, updates)
      console.log(`📁 Oyun kategorisi değiştirildi: ${gameId} → ${newCategory}`)
      return response.data
    } catch (error) {
  ...
  async getLibraryGamesWithDetails(filters = {}) {
    try {
      ...
      do {
        const resp = await libraryApi.getLibrary(this.currentUser.id, { ...filters, page, limit: PAGE_LIMIT })
        const payload = resp?.data ?? resp
        const pageEntries = payload?.entries ?? []
        const pagination = payload?.pagination ?? { page, limit: PAGE_LIMIT, totalPages: 1 }
      ...
      const formatted = rawEntries.map(entry => {
        ...
        const campaigns = Array.isArray(tags) ? [] : (tags?.selectedCampaigns || tags?.campaigns || [])
        const selectedDlcs = Array.isArray(tags) ? [] : (tags?.selectedDlcs || [])
        const steamDlcs = Array.isArray(tags) ? [] : (tags?.steamDlcs || [])
        const gameVariants = Array.isArray(tags) ? undefined : tags?.gameVariants
  ...
  async getGameById(gameId) {
    try {
      ...
      const response = await libraryApi.getEntry(this.currentUser.id, gameId.toString())
      return response.data
    } catch (error) {
