...
class IGDBApiService {
  ...
  async initializeCredentials() {
    try {
      ...
      if (dbCredentials && dbCredentials.clientId && dbCredentials.accessToken) {
      ...
      const localClientId = localStorage.getItem('igdb_client_id') || localStorage.getItem('igdbClientId')
      const localAccessToken = localStorage.getItem('igdb_access_token') || localStorage.getItem('igdbAccessToken')
      
      if (localClientId && localAccessToken) {
        // Migrate to database
        try {
          await apiKeyService.setIgdbCredentials(localClientId, localAccessToken, null, true) // Set as global key
          ...
          console.warn('Failed to migrate IGDB credentials to database:', error)
          // Use them anyway
          this.clientId = localClientId
          this.accessToken = localAccessToken
          this.isInitialized = true
      ...
      console.warn('Failed to initialize IGDB credentials from database:', error)
    }
  }
  ...
  async clearCredentials() {
    try {
      ...
      const clientIdKey = keys.data?.find(key => key.serviceName === 'igdb_client_id')
      const accessTokenKey = keys.data?.find(key => key.serviceName === 'igdb_access_token')
  ...
  async makeRequest(endpoint, body) {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          if (response.status === 429) {
            ...
            const backoffMs = this.applyJitter(retryAfterSec ? retryAfterSec * 1000 : 800 * Math.pow(2, attempt))
            await this.sleep(backoffMs)
            lastError = new Error(`IGDB API Error: 429 Too Many Requests (attempt ${attempt + 1}/${maxRetries + 1})`)
            // Bir sonraki denemeye geç
            continue
          }
          // Diğer hatalar için direkt fırlat
          const errorData = await response.text()
          throw new Error(`IGDB API Error: ${response.status} ${response.statusText} - ${errorData}`)
        }
    ...
    this.logApiCall(endpoint, false, Date.now() - startTime, lastError ? lastError.message : 'Unknown error')
    ...
    try {
      ...
      if (gameName) {
        const nameSearchBody = `
          ...
          limit 100;
        `
        const nameSearchResults = await this.makeRequest('games', nameSearchBody)
        console.log('🔎 İsim araması sonuçları:', nameSearchResults)
        
        // Ana oyun dışındaki sonuçları DLC olarak ekle
        const additionalDlcs = (nameSearchResults || []).filter(game => 
          game.id !== gameId && 
  ...
  getApiStats() {
    ...
    const last30DaysLogs = logs.filter(log => new Date(log.timestamp) > last30Days)
    ...
    return {
      ...
      last7Days: {
        ...
        avgResponseTime: last7DaysLogs.length > 0 
          ? Math.round(last7DaysLogs.reduce((sum, log) => sum + log.responseTime, 0) / last7DaysLogs.length)
          : 0
      },
      last30Days: {
        ...
        avgResponseTime: last30DaysLogs.length > 0 
          ? Math.round(last30DaysLogs.reduce((sum, log) => sum + log.responseTime, 0) / last30DaysLogs.length)
          : 0
      },
  ...
  getEndpointStats(logs) {
    ...
    Object.keys(endpointStats).forEach(endpoint => {
      ...
      endpointStats[endpoint].avgResponseTime = endpointLogs.length > 0
        ? Math.round(endpointLogs.reduce((sum, log) => sum + log.responseTime, 0) / endpointLogs.length)
        : 0
    })
