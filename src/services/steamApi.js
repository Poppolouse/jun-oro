...
class SteamApiService {
  ...
  async makeRequest(url, timeout = 10000) {
    ...
    for (let i = 0; i < this.corsProxies.length; i++) {
      ...
      const needsEncoding =
        proxy.endsWith("?") || proxy.includes("allorigins.win");
      const proxyUrl = needsEncoding
    ...
    try {
      ...
      if (trimmed.startsWith("http://") || trimmed.startsWith("https://")) {
        ...
        if (vanityMatch) {
          ...
          if (!resolved.success)
            throw new Error(resolved.message || "Vanity çözümlenemedi");
          return {
            success: true,
  ...
  async getOwnedGames(steamId) {
    try {
      ...
      if (error.message.includes("403")) {
        console.warn(
          "⚠️ Steam owned games hatası (403): Profil gizli olabilir veya API anahtarı gerekli",
        );
  ...
  async getGameDetails(appId) {
    try {
      ...
      return {
        ...
        name: gameData.name,
        description:
          gameData.short_description || gameData.detailed_description || "",
  ...
  async getGameDLCs(appId) {
    try {
      ...
      if (error.message.includes("400")) {
        console.warn(
          "⚠️ Steam DLC alma hatası (HTTP 400): Geçersiz App ID veya API limiti",
        );
      } else if (error.message.includes("403")) {
        console.warn(
          "⚠️ Steam DLC alma hatası (HTTP 403): Erişim reddedildi - API anahtarı gerekli olabilir",
        );
      } else if (error.message.includes("429")) {
  ...
  async getDLCsForGame(igdbGameName) {
    try {
      ...
      if (error.message.includes("400") || error.message.includes("404")) {
        console.warn(
          "⚠️ IGDB oyunu için Steam DLC alma hatası: Oyun Steam'de bulunamadı",
        );
      } else {
        console.error(
          "❌ IGDB oyunu için Steam DLC alma hatası:",
          error.message,
        );
      }
      return [];
