...
export const ActiveSessionProvider = ({ children }) => {
  ...
  const updateGamePlaytime = async (gameId, additionalTime) => {
    ...
    try {
      ...
      if (game) {
        ...
        console.log(
          "📋 Kütüphanedeki oyunlar:",
          library.entries?.map((g) => ({
            gameId: g.gameId,
            name: g.name || g.title,
          })) || [],
        );
  ...
  const startSession = async (game) => {
    ...
    try {
      ...
      console.log(
        "🎮 Oyun oturumu yerel olarak başlatıldı:",
        game.name || game.title,
      );
  ...
  const stopSession = async (
    ...
    console.log("🛑 stopSession çağrıldı:", {
    ...
    if (activeSession && user) {
      ...
      console.log("📊 Session verileri:", {
        oyun:
          activeSession.gameName || activeSession.name || activeSession.title,
      ...
      console.log(
        "✅ Oyun oturumu sonlandırıldı:",
        completedSessionData.gameName ||
          completedSessionData.name ||
          completedSessionData.title,
        "Süre:",
        formatTime(actualPlayTime),
      );
    } else {
  ...
  const toggleSession = async () => {
    ...
    if (isRunning) {
      ...
      if (
        ...
        !pauseHistory[pauseHistory.length - 1].resumedAt
      ) {
        ...
        const totalPausedTime = pauseHistory.reduce((total, pause) => {
          if (pause.pausedAt && pause.resumedAt) {
            const pauseDuration =
              (new Date(pause.resumedAt) - new Date(pause.pausedAt)) / 1000;
    ...
    if (savedSession) {
      try {
        ...
        if (session.pauseHistory && session.pauseHistory.length > 0) {
          const lastPause =
            session.pauseHistory[session.pauseHistory.length - 1];
        ...
        const adjustedElapsedSeconds = Math.max(
          0,
          elapsedSeconds - totalPausedTime,
        );
        ...
        console.log(
          "🔄 Aktif oturum geri yüklendi:",
          session.gameName || session.name || session.title,
          "Kullanıcı:",
          user.username,
        );
      } catch (error) {
