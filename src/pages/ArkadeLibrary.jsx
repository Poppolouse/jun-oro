...
function ArkadeLibrary() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#0a0a0a] via-[#1a1a2e] to-[#16213e] flex flex-col" id="arkade-library" data-registry="2.1">
      <ArkadeHeader />
      <div className="flex flex-1 min-h-0" id="library-layout" data-registry="2.1.B">
        <ArkadeSidebar />
        <LibraryContent showCampaignCards={showCampaignCards} setShowCampaignCards={setShowCampaignCards} />
        <LibraryRightSidebar showCampaignCards={showCampaignCards} setShowCampaignCards={setShowCampaignCards} />
      </div>
      <SiteFooter />
      <ElementSelector />
    </div>
...
function LibraryContent({ showCampaignCards, setShowCampaignCards }) {
  ...
  const deriveDefaultPlaytimeMinutes = (gameData) => {
    try {
      const campaigns = Array.isArray(gameData?.campaigns) ? gameData.campaigns : []
      if (campaigns.length > 0) {
        const preferred = ['main story', 'main + extra', 'completionist']
        const pick = campaigns.find(c => preferred.includes((c?.name || '').toLowerCase())) || campaigns[0]
        ...
        if (hltb) {
          return (
            ...
            parseDurationToMinutes(hltb.completionist) || 0
          )
        }
      }
      return 0
  ...
  const handleGameAdded = async (gameData) => {
    try {
      if (editTarget) {
        ...
        const updatedList = await userLibrary.getLibraryGamesWithDetails() || []
        ...
        const category = categoryMapping[(gameData.status || '').toLowerCase()] || 'playing'
        ...
        if (added) {
          ...
          if (defaultMins > 0) {
            await userLibrary.updateGameDetails(String(gameData.id), { playtime: defaultMins })
          }
        }
  ...
  const openEdit = (g) => {
    const category = (g.libraryInfo?.category || g.category || 'wishlist').toLowerCase()
    const statusMapping = {
      playing: 'playing',
      completed: 'completed',
      wishlist: 'backlog',
    ...
    const releaseYear = g.first_release_date ? new Date((typeof g.first_release_date === 'number' ? g.first_release_date * 1000 : Date.now())).getFullYear() : null
    const primaryPlatform = Array.isArray(g.platforms) && g.platforms.length > 0 ? (g.platforms[0].name || g.platforms[0]) : ''
    const primaryGenre = Array.isArray(g.genres) && g.genres.length > 0 ? (g.genres[0].name || g.genres[0]) : ''
    const coverUrl = (() => {
      const c = g.cover
      if (c?.image_id) return `https://images.igdb.com/igdb/image/upload/t_thumb/${c.image_id}.jpg`
  ...
  const getDeveloper = (g) => {
    ...
    if (typeof g.developer === 'string' && g.developer.trim()) return g.developer.trim()
    if (typeof g?.steamData?.developer === 'string' && g.steamData.developer.trim()) return g.steamData.developer.trim()
    if (Array.isArray(g.developers) && g.developers.length && typeof g.developers[0] === 'string') return g.developers[0]

    // 3) Kampanya customProperties ("Geliştirici")
    const cp = Array.isArray(g.campaigns) ? g.campaigns : []
    for (const c of cp) {
      const props = c?.customProperties || []
      const p = props.find((p) => (p.name || '').toLowerCase() === 'geliştirici')
  ...
  const renderGameCard = (g, i) => {
    const coverUrl = (() => {
      const c = g.cover
      if (c?.image_id) return `https://images.igdb.com/igdb/image/upload/t_1080p/${c.image_id}.jpg`
      if (typeof c === 'string') return c.replace('t_cover_small', 't_1080p').replace('t_thumb', 't_1080p')
      if (typeof c?.url === 'string') return c.url.replace('t_cover_small', 't_1080p').replace('t_thumb', 't_1080p')
      return '/api/placeholder/160/240?text=Arkade'
    })()
    const title = g.name || g.title || 'Bilinmeyen Oyun'
    const platform = Array.isArray(g.platforms) && g.platforms.length > 0 ? (g.platforms[0].name || g.platforms[0]) : null
    const genres = Array.isArray(g.genres) && g.genres.length > 0 ? (g.genres[0].name || g.genres[0]) : null
    ...
    return (
      <div 
        ...
        className={`group relative w-[230px] h-[346px] rounded-xl overflow-hidden bg-white/5 border transition cursor-pointer ${
          checked ? 'border-emerald-500/60 ring-2 ring-emerald-500/30' : 'border-white/10 hover:border-emerald-500/40'
        }`}
        onClick={(e) => {
      ...
      >
        <img src={coverUrl} alt={title} className="absolute inset-0 w-full h-full object-cover transition-transform duration-300 group-hover:scale-[1.03]" />
        ...
        <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          <button
            ...
            title={activeSession?.id === g.id ? 'Zaten oynuyor' : (g.campaigns && g.campaigns.length > 0 ? 'Campaign seç ve başlat' : 'Oturumu başlat')}
          >
            {activeSession?.id === g.id ? (
        ...
        <div className="absolute bottom-0 left-0 right-0 p-3 bg-gradient-to-t from-black/65 to-transparent">
          ...
          <div className="mt-1 flex items-center justify-between text-[11px]">
            <span className="text-gray-300 truncate max-w-[50%]">{developer || '—'}</span>
            <span className="px-2 py-0.5 rounded-full bg-white/10 border border-white/20 text-white/90">{statusLabel}</span>
          </div>
          <div className="mt-1 flex items-center justify-between text-[10px]">
            <span className="text-gray-300">{formatPlaytime(g.libraryInfo?.playtime)}</span>
  ...
  const renderLibraryListContent = () => {
    let filtered = games || []
    if (filterOption !== 'all') {
      filtered = filtered.filter(g => ((g.libraryInfo?.category || g.category || '').toLowerCase()) === filterOption)
    }
    if (searchText.trim()) {
      const q = searchText.toLowerCase()
      filtered = filtered.filter(g => (g.name || g.title || '').toLowerCase().includes(q))
    }
    if (sortOption === 'name') {
      ...
      filtered = [...filtered].sort((a, b) => (b.libraryInfo?.addedAt || 0) - (a.libraryInfo?.addedAt || 0))
    }

    // Seçim bilgileri (ileride kullanılabilir)
    const allSelected = filtered.length > 0 && filtered.every(f => selectedIds.has(String(f.id)))
    const selectedCount = Array.from(selectedIds).filter(id => filtered.some(f => String(f.id) === id)).length
    ...
    if (viewOption === 'list') {
      return (
        <div className="space-y-3">
          {filtered.map((g, i) => (
            <div key={g.id || i} className="flex items-center gap-4 p-3 bg-white/5 rounded-lg border border-white/10">
              <img src={(g.cover && (g.cover.url || g.cover)) || '/api/placeholder/80/100?text=Arkade'} alt={g.name || g.title} className="w-12 h-16 rounded object-cover" />
              <div className="flex-1">
                <div className="text-white font-medium">{g.name || g.title}</div>
                <div className="text-xs text-gray-400">{g.libraryInfo?.category || g.category}</div>
              </div>
              <div className="text-xs text-gray-400">{g.rating ? `⭐ ${Math.round(g.rating)}` : ''}</div>
            </div>
          ))}
        </div>
      )
    ...
    return (
      <>
        ...
        {totalPages > 1 && (
          <div className="mt-4 flex items-center justify-between" id="library-pagination" data-registry="2.1.B1.2.3">
            <div className="text-xs text-gray-300" id="pagination-page-label" data-registry="2.1.B1.2.3.0">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                id="pagination-prev-btn" data-registry="2.1.B1.2.3.1"
              >Önceki</button>
              {/* Sayfa numaraları (QoL: ellipsis ile) */}
              <div className="flex items-center gap-1" id="pagination-number-list" data-registry="2.1.B1.2.3.3">
                {(() => {
                  const range = (() => {
                    ...
                    if (typeof item !== 'number') {
                      return (
                        <span
                          ...
                          id={item === 'left-ellipsis' ? 'pagination-ellipsis-left' : 'pagination-ellipsis-right'}
                          data-registry={item === 'left-ellipsis' ? '2.1.B1.2.3.3.l' : '2.1.B1.2.3.3.r'}
                        >…</span>
                      )
                    }
              ...
              <button
                ...
                id="pagination-next-btn" data-registry="2.1.B1.2.3.2"
              >Sonraki</button>
            </div>
          </div>
        )}
  ...
  return (
    <div className="flex-1 p-8 pr-4 overflow-y-auto" id="library-content" data-registry="2.1.B1">
      {/* Page Header + Tab Switcher */}
      <div className="mb-6" id="library-header" data-registry="2.1.B1.1">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <h1 className="text-2xl font-bold text-white" id="library-title" data-registry="2.1.B1.1.1">Oyun Kütüphanesi</h1>
            <span className="text-sm text-gray-400" id="game-count" data-registry="2.1.B1.1.2">Toplam {games?.length || 0} oyun</span>
          </div>
          <div className="flex items-center gap-2" id="tab-controls" data-registry="2.1.B1.1.3">
            <button
              ...
              id="games-tab" data-registry="2.1.B1.1.3.1"
            >
              🎮 Oyunlar
            </button>
            <button
              ...
              id="cycle-tab" data-registry="2.1.B1.1.3.2"
            >
              🌀 Döngü Planlayıcısı
            </button>
          ...
          <button
              ...
              id="add-game-btn" data-registry="2.1.B1.1.3.3"
            >
              + Oyun Ekle
            </button>
            <button
              ...
              id="steam-import-btn" data-registry="2.1.B1.1.3.4"
            >
              Steam'den İçe Aktar
            </button>
          </div>
        ...
        {lastAdded && (
          <div className="mt-2 text-sm text-emerald-400" id="last-added-info" data-registry="2.1.B1.1.4">Kütüphaneye eklendi: {lastAdded}</div>
        )}
      </div>
      ...
      {activeTab === 'games' ? (
        <div id="games-tab-content" data-registry="2.1.B1.2">
          {/* Search + Filters */}
          <div className="bg-white/5 backdrop-blur-md rounded-xl p-4 border border-white/10 mb-4" id="filters-panel" data-registry="2.1.B1.2.1">
            <div className="flex items-center gap-3">
              <input
                value={searchText}
                ...
                id="search-input" data-registry="2.1.B1.2.1.1"
              />
              <select 
                value={sortOption} 
                onChange={(e) => setSortOption(e.target.value)} 
                className="bg-white/5 border border-white/10 rounded-lg p-3 text-white text-sm"
                id="sort-select" data-registry="2.1.B1.2.1.2"
              >
                <option value="recent" className="bg-gray-800">İtem Sırala: Son eklenen</option>
                <option value="name" className="bg-gray-800">İtem Sırala: Ada göre</option>
                <option value="rating" className="bg-gray-800">İtem Sırala: Puan</option>
              </select>
              <select 
                value={filterOption} 
                onChange={(e) => setFilterOption(e.target.value)} 
                className="bg-white/5 border border-white/10 rounded-lg p-3 text-white text-sm"
                id="filter-select" data-registry="2.1.B1.2.1.3"
              >
                <option value="all" className="bg-gray-800">İtem Filtreleme: Hepsi</option>
              ...
              <select 
                ...
                className="bg-white/5 border border-white/10 rounded-lg p-3 text-white text-sm"
                id="view-select" data-registry="2.1.B1.2.1.4"
              >
                <option value="grid" className="bg-gray-800">İtem Görünüm: Grid</option>
                <option value="list" className="bg-gray-800">Liste</option>
              </select>
            </div>
            
            {/* Toplu seçim kontrolleri */}
            <div className="flex items-center gap-6 ml-auto mt-3" id="bulk-controls" data-registry="2.1.B1.2.1.5">

              {(() => {
                let filtered = games || []
                if (filterOption !== 'all') {
                  filtered = filtered.filter(g => (g.libraryInfo?.category || g.category) === filterOption)
                }
                if (searchText.trim()) {
                  const q = searchText.toLowerCase()
                  filtered = filtered.filter(g => (g.name || g.title || '').toLowerCase().includes(q))
                ...
                const handleSelectAllChange = (checked) => {
                  if (checked) {
                    ...
                    const next = new Set(Array.from(selectedIds).filter(id => !filtered.some(f => String(f.id) === id)))
                    setSelectedIds(next)
                  }
                }
                
                const triggerBulkDelete = () => {
                  const idsToDelete = Array.from(selectedIds).filter(id => filtered.some(f => String(f.id) === id))
                ...
                return (
                  <>
                    <label className="inline-flex items-center gap-2 text-xs text-white/80">
                      ...
                      {selectedCount > 0 && (
                        <span className="ml-2 px-2 py-0.5 rounded-full bg-white/10 border border-white/20 text-white/80">{selectedCount} seçili</span>
                      )}
                    </label>
                    <button
          ...
          <div ref={listRef} className="bg-white/5 backdrop-blur-md rounded-2xl p-6 border border-white/10 min-h-[300px]" id="games-list" data-registry="2.1.B1.2.2">
            {loading ? (
        ...
        <LibraryCyclePlanner games={games} onAdd={() => setIsAddOpen(true)} renderCard={renderGameCard} />
      )}
          ...
          <div className="w-full max-w-md bg-[#111827] border border-white/10 rounded-xl overflow-hidden">
            <div className="p-4 border-b border-white/10">
              ...
              <div className="text-gray-400 text-sm mt-1">
                {deleteIds.length > 1 ? `${deleteIds.length} oyun için işlem seçin:` : 'Bu oyun için işlem seçin:'}
              </div>
            </div>
            <div className="p-4 space-y-3">
              ...
              <button
                onClick={() => { setShowDeleteConfirm(false); setDeleteIds([]) }}
      ...
      <div className="bg-gradient-to-br from-[#00ff88]/10 via-purple-500/10 to-blue-500/10 backdrop-blur-xl rounded-2xl p-8 border border-white/10 shadow-xl relative overflow-hidden h-full">
        ...
        <div className="relative z-10 text-center h-full flex flex-col justify-center">
          ...
          <p className="text-gray-300 text-lg mb-6 leading-relaxed">
            Kütüphane yönetimi, istatistikler ve daha fazlası için harika yenilikler geliyor!
          </p>
          ...
          <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4 border border-white/20">
            <div className="flex items-center justify-center gap-2 text-[#00ff88]">
              ...
              <span className="text-sm font-medium">Aktif Geliştirme Aşamasında</span>
            </div>
...
function LibraryCyclePlanner({ games, onAdd, renderCard }) {
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="bg-white/5 backdrop-blur-md rounded-2xl p-8 border border-white/10 text-center max-w-md">
          ...
          <h3 className="text-xl font-semibold text-white mb-2">Henüz Çalışma Altında</h3>
          <p className="text-gray-400 mb-4">
            Cycle Planner özelliği şu anda geliştirme aşamasındadır. Bu özellik yakında kullanıma sunulacaktır.
          </p>
  ...
  const completed = games?.filter(g => (g.libraryInfo?.category || g.category) === 'completed').length || 0
  const initialCycles = [
  ...
  const getGameCategory = (g) => (g.libraryInfo?.category || g.category || 'wishlist')
  ...
  const addFromBacklogToSelected = (count = 3) => {
    ...
    setCycles(prev => {
      ...
      const backlogIdxs = games
        .map((g, i) => ({ g, i }))
        .filter(({ g, i }) => getGameCategory(g) === 'wishlist' && !existing.has(i))
        .map(({ i }) => i)
  ...
  const addNewCycle = () => {
    setCycles(prev => [...prev, { name: `Cycle ${prev.length + 1}`, status: 'Planlandı', items: [] }])
  }
  return (
    <div className="space-y-6">
      ...
      >
          ...
          <React.Fragment key={c.name}>
            ...
            >
              <div className="flex items-center justify-between mb-2">
                ...
                <span className={`text-xs ${c.status === 'Aktif' ? 'text-emerald-400' : 'text-yellow-300'}`}>{c.status}</span>
              </div>
              <div className="text-sm text-gray-300">{c.items.length} / {c.items.length} oyun</div>
              <div className="flex justify-between text-xs text-gray-500 mt-1">
                <span>
                  {
                    (() => {
                      const compInCycle = c.items.filter(i => getGameCategory(games[i]) === 'completed').length
                      return total ? Math.round((compInCycle / Math.max(1, c.items.length)) * 100) : 0
                    })()
                  }% tamamlandı
                </span>
                <span>{Math.max(0, c.items.length - c.items.filter(i => getGameCategory(games[i]) === 'completed').length)} kaldı</span>
              </div>
            </div>
            {idx < cycles.length - 1 && (
        ...
        {/* New cycle */}
        <div onClick={addNewCycle} className="cycle-card min-w-[240px] snap-start bg-white/5 rounded-xl p-4 border border-white/10 flex items-center justify-center cursor-pointer hover:border-emerald-500/40">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="bg-white/5 rounded-xl p-6 border border-white/10">
          <div className="text-lg font-bold text-white mb-2">{cycles[selectedIdx]?.name || 'Cycle'}</div>
          <div className="text-emerald-400 font-bold text-2xl">{cycles[selectedIdx]?.items.length || 0} / {cycles[selectedIdx]?.items.length || 0}</div>
          <div className="text-gray-400 text-sm mt-1">oyun</div>
          <button onClick={renameSelectedCycle} className="mt-4 px-3 py-2 bg-white/10 text-white rounded-lg text-sm">İsim Değiştir</button>
        </div>
        <div className="lg:col-span-2">
          <div className="bg-white/5 rounded-xl p-6 border border-white/10">
            <div className="flex items-center gap-3 mb-4">
              <input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Oyun ara..." className="flex-1 bg-white/5 border border-white/10 rounded-lg p-3 text-white placeholder-gray-400 text-sm" />
              <button onClick={() => setIsPickerOpen(true)} className="px-3 py-2 bg-emerald-500/20 text-emerald-300 rounded-lg text-sm border border-emerald-500/40">Kütüphaneden Ekle</button>
              <button onClick={() => addRandomToSelected(3)} className="px-3 py-2 bg-white/10 text-white rounded-lg text-sm">Rastgele</button>
              <button onClick={() => addFromBacklogToSelected(3)} className="px-3 py-2 bg-white/10 text-white rounded-lg text-sm">Backlog'dan</button>
            </div>
            ...
            <div className="flex flex-wrap gap-4">
              {
                (() => {
                  ...
                  const items = indices
                    .map(i => ({ g: games[i], i }))
                    .filter(({ g }) => !!g)
                    .filter(({ g }) => (g.name || g.title || '').toLowerCase().includes(searchQuery.toLowerCase()))
                  if (items.length === 0) {
                    return <div className="text-gray-400 text-sm">Bu cycle için henüz oyun eklenmedi.</div>
                  }
                  return items.map(({ g, i }) => (
                    renderCard ? renderCard(g, i) : (
                      <div key={g.id || i} className="group relative w-[160px] h-[240px] rounded-xl overflow-hidden bg-white/5 border border-white/10">
                        <img src={(g.cover && (g.cover.url || g.cover)) || '/api/placeholder/160/240?text=Arkade'} alt={g.name || g.title} className="absolute inset-0 w-full h-full object-cover" />
                        <div className="absolute inset-0 bg-gradient-to-t from-black/75 via-black/40 to-transparent" />
                        <div className="absolute bottom-0 left-0 right-0 p-3">
                          <div className="text-white text-sm font-semibold truncate">{g.name || g.title}</div>
                          <div className="text-[10px] text-gray-300">{(g.libraryInfo?.category || g.category || 'wishlist')}</div>
                        </div>
                      </div>
                    )
                  ))
          ...
          <div className="w-full max-w-3xl bg-[#111827] border border-white/10 rounded-xl overflow-hidden">
            <div className="p-4 border-b border-white/10 flex items-center gap-3">
              <input
                ...
                value={searchQuery}
              />
              <button onClick={() => setIsPickerOpen(false)} className="px-3 py-2 text-sm rounded-lg bg-white/10 text-white">Kapat</button>
              <button
                onClick={() => {
                  ...
                  setCycles(prev => {
                    ...
                    const existing = new Set(target.items)
                    const added = Array.from(pickerSelection).filter(i => !existing.has(i))
                    next[selectedIdx] = { ...target, items: [...target.items, ...added] }
                    return next
                  })
                  ...
                  return (
                    ...
                    >
                      <div className="w-full aspect-[2/3] overflow-hidden">
                        <img src={coverUrl} alt={g.name || g.title} className="w-full h-full object-cover" />
                      </div>
                      <div className="absolute bottom-0 left-0 right-0 p-2 bg-gradient-to-t from-black/70 to-transparent">
                        <div className="text-white text-xs font-medium line-clamp-1">{g.name || g.title}</div>
                        <div className="text-[10px] text-gray-400">{g.libraryInfo?.category || g.category || 'wishlist'}</div>
                      </div>
                    </button>
                  )
                })
