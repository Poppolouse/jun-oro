...
function checkTermInText(searchTerm, text) {
  if (!searchTerm || !text) return { contains: false, position: -1, exactMatch: false }
...
export function calculateRelevanceScore(game, searchTerm) {
  ...
  if (game.summary) {
    ...
    if (summaryCheck.contains) {
      ...
      const termCount = (game.summary.toLowerCase().match(new RegExp(term, 'g')) || []).length
...
export function formatRelevanceScore(score) {
  if (score >= 80) {
    return { percentage: score, label: 'Mükemmel Eşleşme', color: 'text-green-600' }
  } else if (score >= 60) {
    return { percentage: score, label: 'İyi Eşleşme', color: 'text-blue-600' }
  } else if (score >= 40) {
    return { percentage: score, label: 'Orta Eşleşme', color: 'text-yellow-600' }
  } else if (score >= 20) {
    return { percentage: score, label: 'Zayıf Eşleşme', color: 'text-orange-600' }
  } else {
