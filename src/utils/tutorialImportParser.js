...
const resolvePagePath = (categoryId, pageId) => {
  const category = Object.values(PAGE_CATEGORIES).find(c => c.id === categoryId)
...
const mapType = (value) => {
  ...
  if (v === 'page_guide' || v === 'page-guide') return TUTORIAL_TYPES.PAGE_GUIDE
...
function parseDSL(text) {
  ...
  const assignNested = (obj, keyPath, rawValue) => {
    ...
    } else if (rawValue && rawValue.trim().startsWith('{') || rawValue.trim().startsWith('[')) {
      // JSON string
      try {
  ...
  for (const rawLine of lines) {
    ...
    switch (section) {
      ...
      case 'SETTINGS': {
        if (key.startsWith('overlay.') || key.startsWith('highlight.') || key.startsWith('modal.')) {
          assignNested(tutorial.settings, key, value)
        } else {
          if (key === 'autoStart') tutorial.settings.autoStart = toBoolean(value)
          else if (key === 'showProgress') tutorial.settings.showProgress = toBoolean(value)
          else if (key === 'allowSkip') tutorial.settings.allowSkip = toBoolean(value)
        }
        break
      }
      case 'STEP': {
        ...
        else if (key.startsWith('content.')) assignNested(currentStep, key, value)
        break
      }
      default:
  ...
  if (tutorial.targetPage?.categoryId && tutorial.targetPage?.pageId) {
    const resolved = resolvePagePath(tutorial.targetPage.categoryId, tutorial.targetPage.pageId)
    if (resolved) {
      tutorial.targetPage.pageName = tutorial.targetPage.pageName || resolved.pageName
      tutorial.targetPage.pagePath = tutorial.targetPage.pagePath || resolved.pagePath
    }
  }
...
function normalizeTutorial(obj) {
  const tutorial = {
    id: obj.id || (obj.title ? `${slugify(obj.title)}-tutorial` : `tutorial-${Date.now()}`),
    title: obj.title || 'Yeni Tutorial',
    description: obj.description || '',
    version: obj.version || '1.0.0',
    type: mapType(obj.type),
    targetPage: obj.targetPage || null,
    steps: Array.isArray(obj.steps) && obj.steps.length > 0 ? obj.steps.map((s, i) => ({
      ...
      content: {
        ...
        buttons: Array.isArray(s.content?.buttons) && s.content.buttons.length > 0
          ? s.content.buttons
          : [{ text: 'Devam', action: 'next', style: 'primary' }]
      }
    })) : [defaultStep()],
...
export function parseTutorialText(text) {
  ...
  if (!rawObj) {
    return { tutorial: null, errors: errors.length ? errors : ['Geçersiz dosya formatı. JSON veya DSL kullanın.'] }
  }
  ...
  try {
    ...
    if (!Array.isArray(tutorial.steps) || tutorial.steps.length === 0) errors.push('En az bir adım olmalı')
