...
import { validateGameLibraryEntry, validatePagination } from '../utils/validation.js';
...
export async function handleGetUserLibrary(request, env) {
  try {
    ...
    const sortField = allowedSortFields.includes(sortBy) ? 
      (sortBy === 'title' ? 'g.title' : `ugl.${sortBy}`) : 'ugl.added_at';
    ...
    const libraryEntries = await env.DB.prepare(query).bind(...params).all();
